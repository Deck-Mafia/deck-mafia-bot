...
async function createEvent(i: ChatInputCommandInteraction) {
	...
	const canVote = i.options.getBoolean('vote', false) ?? undefined;
	const canBeVoted = i.options.getBoolean('recipient', false) ?? undefined;
...
async function createVoteCount(i: ChatInputCommandInteraction) {
	...
	const closeAt = closeAtStamp ? new Date(closeAtStamp * 1000) : undefined;
	...
	try {
		...
		await i.editReply('Created');

		if (closeAt) await i.followUp(`**Day end <t:${closeAtStamp}:R>**`);
	} catch (err) {
		return i.editReply('An error has occurred. Error handling not yet implemented');
	}
}
